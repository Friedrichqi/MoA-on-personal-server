...
class MATHEvaluator(Evaluator):
    ...
    def check_answers_equiv(self, answer_a: str, answer_b: str):
        ...
        pattern = r'''\\boxed\s*{((?:[^{}]+|{(?1)})*)}'''
        matches = re.findall(pattern, answer_a, re.VERBOSE)
        try:
            answer_a = matches[-1]
        except:
            # print(answer_a)
            ...
            pass
        answer_a = answer_a.replace("dfrac", "frac")

        answer_b = answer_b.strip()
        answer_b = re.sub(r"\\left\s*\(\s*", "(", answer_b)
        answer_b = re.sub(r"\s*\\right\s*\)", ")", answer_b)

        try:
            if parse_latex(answer_a) == parse_latex(answer_b):
                return True
        except Exception as e:
            pass
        if answer_b.lower() in answer_a.lower():
            return True
